...
float Scene::interpolation(const Vec3f &pos, T &res, std::vector<GridType> Grids) const {
...
std::tuple<Vec3f, float, float> Scene::getEmissionOpacity(Vec3f value) const {
    ...
    interpolation(value, emission_inter, moduleGrids);
    auto layer = interpolation(value, opacity_inter, QGrids);
    return {sampleEmission(emission_inter), sampleOpacity(opacity_inter), layer};
}
...
void Scene::genQGrids() {
    for (const auto &grid: vectorGrids) {
        ...
        for (auto iter = grid->cbeginValueOn(); iter; ++iter) {
            ...
            openvdb::Coord low = ijk + openvdb::Coord{-1, -1, -1};
            openvdb::Coord high = ijk + openvdb::Coord{1, 1, 1};
            Vec3f grad = (vec_accessor.getValue(high) - vec_accessor.getValue(low)) / (2 * dx);
            qValue = -0.5f * (grad.dot(grad)) - grad[0] * grad[1] - grad[0] * grad[2] - grad[1] * grad[2];
